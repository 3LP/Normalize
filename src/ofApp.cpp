...
void ofApp::setup(){
    ...
    cam_pos_track_hand = true;  // start tracking hand
...
void ofApp::draw_initial_physics_scene()
{
...
#if CFG_DRAW_BOUNDING_WALLS
    {
        ...
        shapeInfo.position = ktVector(0,0,0);
        shapeInfo.size = ktVector(SZ*2,WD,SZ*2);
        (void)ktMakeShape(shapeInfo, NULL);
        
        shapeInfo.position = ktVector(0,DIS*2,0);
        shapeInfo.size = ktVector(SZ*2,WD,SZ*2);
        (void)ktMakeShape(shapeInfo, NULL);
        
        shapeInfo.position = ktVector(DIS,0,0);
        shapeInfo.size = ktVector(WD,SZH,SZ*2);
        (void)ktMakeShape(shapeInfo, NULL);
        ...
        shapeInfo.position = ktVector(0,0,DIS);
        shapeInfo.size = ktVector(SZ*2,SZH,WD);
        (void)ktMakeShape(shapeInfo, NULL);
...
#if CFG_DRAW_SPHERE
    {
        ...
        shapeInfo.mass = 1.0f;
        shapeInfo.inertia = ktVector(0,0,0);
        shapeInfo.position = ktVector(0,350,-75);
        shapeInfo.size = ktVector(BULLET_SPHERE_RADIUS,BULLET_SPHERE_RADIUS,BULLET_SPHERE_RADIUS);
        (void)ktMakeShape(shapeInfo, NULL);
    }
#endif
...
#if CFG_DRAW_CUBE
    {
        ...
        float start_x = START_POS.x() - ARRAY_SIZE_X / 2;
        float start_y = START_POS.y();
        float start_z = START_POS.z() - ARRAY_SIZE_Z / 2;
        
        for (int k = 0; k < ARRAY_SIZE_Y; k++) {
            for (int i = 0; i < ARRAY_SIZE_X; i++) {
                for (int j = 0; j < ARRAY_SIZE_Z; j++) {
                    
                    shapeInfo.position = ktVector(2.0f * i + start_x,
                                                  10.0f + 2.0f * k + start_y,
                                                  2.0f * j + start_z);
                    (void)ktMakeShape(shapeInfo, NULL);
                }
            }
...
#if CFG_DRAW_CYLINDER
    {
        ...
        shapeInfo.inertia = ktVector(0,0,0);
        shapeInfo.position = ktVector(0,350,-75);
        shapeInfo.size = ktVector(10,20,30);
        (void)ktMakeShape(shapeInfo, NULL);
    }
#endif
...
void ofApp::draw(){
    ...
    stringstream ss;
    ss << "3D.G(" << APP_VERSION << ") FR: " << ofToString(ofGetFrameRate(),0) << "hz, Leap_API FR: "
...
//               t.orientation.x, t.orientation.y, t.orientation.z, t.orientation.w );
    ...
    cam.begin();
    {
        ...
        if (bInfoText) {
            ...
            ss << "(g): View tracks right hand: " << (cam_pos_track_hand?"TRUE":"FALSE") << endl;
            ...
            ss << "(1/2)(4/5)(7/8): campos(" << campos.x << "," << campos.y << "," << campos.z << ")" << endl;
...
void ofApp::keyPressed(int key){
    ...
    switch (key)
    {
        ...
        case 'f':
            ...
            break;
        case 's':
            // we restart the world but give it 30 draw() frames for bullet, etc
